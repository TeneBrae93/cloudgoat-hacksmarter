...
output "secret_key" {
  value     = nonsensitive(aws_iam_access_key.r_waterhouse.secret)
  sensitive = false
}
...
output "user1_secret_key" {
  value     = nonsensitive(aws_iam_access_key.canarytoken_user.secret)
  sensitive = false
}
...
output "user2_secret_key" {
  value     = nonsensitive(aws_iam_access_key.spacecrab_user.secret)
  sensitive = false
}
...
output "user3_secret_key" {
  value     = nonsensitive(aws_iam_access_key.spacesiren_user.secret)
  sensitive = false
}
#AWS Account ID
